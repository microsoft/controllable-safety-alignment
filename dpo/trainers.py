...
class BasicTrainer(object):
    ...
    def train(self):
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                if self.example_counter > 0:
                    if self.config.debug:
                        ...
                        if not self.config.only_save_final:
                            output_dir = os.path.join(self.run_dir, f'step-{self.example_counter}')
                            rank0_print(f'creating checkpoint to write to {output_dir}...')
                            self.save(output_dir, mean_eval_metrics)
                        else:
                            rank0_print('skipping save due to only_save_final=True')
            #### END EVALUATION ####
        ...
        # jack added; end training with another round of evaluation+saving
        rank0_print(f'Running evaluation after {self.example_counter} train examples')
        self.policy.eval()
    ...
    def save(self, output_dir: Optional[str] = None, metrics: Optional[Dict] = None):
        ...
        rank0_print(f'saved policy and tokenizer to {output_dir}!')
...
class FSDPTrainer(BasicTrainer):
    ...
    def save(self, output_dir=None, metrics=None):
        ...
        # only save policy to save space
